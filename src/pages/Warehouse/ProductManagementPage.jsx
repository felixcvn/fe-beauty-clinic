...
const ProductManagementPage = () => {
    ...
    return (
        <div className="space-y-6 md:space-y-10 animate-fade-in pb-12 px-4 md:px-0">
                                ...
                                <tr key={p.id} className="group hover:bg-primary/[0.02] transition-colors">
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => { setEditingProduct(p); setIsModalOpen(true); }} className="p-2 rounded-xl text-primary/40 hover:bg-white hover:text-primary transition-all"><Edit3 className="w-4 h-4" /></button>
                                            <button onClick={() => setDeleteConfirm({ open: true, id: p.id, name: p.name })} className="p-2 rounded-xl text-red-400 hover:bg-white hover:text-red-500 transition-all"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                        ...
                        <div key={p.id} className="p-6 space-y-4">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1">
                                    <span className="text-[10px] font-black text-primary/30 uppercase tracking-widest block mb-1">{p.id}</span>
                                    <h4 className="text-base font-black text-primary tracking-tight leading-tight mb-2">{p.name}</h4>
                                    <span className="px-2 py-1 rounded-lg bg-secondary text-[8px] font-black text-primary uppercase tracking-widest">{p.category}</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 bg-gray-50 p-4 rounded-2xl border border-primary/5">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-xs font-black text-primary tracking-tight">Rp {p.price.toLocaleString('id-ID')}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[8px] font-black text-primary/40 uppercase tracking-widest">Stok</p>
                                    <p className={`text-xs font-black tracking-tight ${p.stock < 15 ? 'text-red-500' : 'text-primary'}`}>{p.stock} Unit</p>
                                </div>
                            </div>
            ...
            {/* Portal Delete Confirm tetap sama */}
